...
export default function FAQItem({ title, key, text }) {
    const [showDropDown, setShowDropDown] = useState(false);
    const [index, setIndex] = useState(null);
    return (
        <AnimatePresence>
        <motion.div layout layoutId={key}
                ...
                <div className='flex gap-[1em] items-center lg:gap-[2.9em] w-full   py-[1em] lg:py-[1.4em]'
                    ...
                    <FontAwesomeIcon icon={showDropDown ? faMinus : faPlus} className='text-blackgray md:text-[1.2em] cursor-pointer' onClick={() => { setIndex(key); setShowDropDown(!showDropDown) }} />
                    <p className='font-sans text-[1em] sm:text-[1em] lg:text-[1.1em] font-[600] text-blackgray' >{title}</p>
                </div>
                     {showDropDown && index === key && <motion.div
                        key={key}
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0.25 }}
